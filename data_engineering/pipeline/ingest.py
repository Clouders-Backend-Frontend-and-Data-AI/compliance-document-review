...
def ingest_document(document_id: str, file_bytes: bytes, filename: str) -> dict:
    ...
    Requires Postgres + pgvector (document_chunks table).
    """
    from data_engineering.config import get_settings

    if not get_settings().uses_pgvector:
        raise RuntimeError(
            "ingest_document requires a PostgreSQL DATABASE_URL with pgvector support"
        )
