...
PATTERNS: list[tuple[re.Pattern, EntityType]] = [
    ...
    # Prefixed account / folio identifiers (e.g. ACCT-998877, FOLIO-445566)
    (
        re.compile(r"\b(?:ACCT|FOLIO|POLICY|IRA)[-#:\s]*[A-Z0-9\-]{5,20}\b", re.IGNORECASE),
        EntityType.ACCOUNT,
    ),
    # Account numbers (10–18 digit sequences, optionally hyphenated)
    (
        re.compile(r"\b(?:account|acct|acc|portfolio(?:\s*id)?)[\s#.:]*([A-Z0-9\-]{5,20})\b", re.IGNORECASE),
        EntityType.ACCOUNT,
    ),
    (
