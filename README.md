...
- [DevOps & Production Infrastructure](#-devops--production-infrastructure)
  - [DevOps Architecture](#-devops-architecture)
  - [DevOps Technology Stack](#-devops-technology-stack)
  ...
  - [DevOps Engineer Contribution](#-devops-engineer-contribution)
  - [DevOps Definition of Done](#-devops-definition-of-done)
...
git clone https://github.com/Clouders-Backend-Frontend-and-Data-AI/compliance-document-review.git
...
## ☁️ DevOps & Production Infrastructure

This project includes a production-oriented DevOps implementation covering containerization, infrastructure as code, CI/CD, cloud security, deployment automation, and monitoring.
...
compliance-backend — FastAPI application
compliance-postgres — PostgreSQL with pgvector

The production PostgreSQL service is kept on the internal Docker network and is not exposed publicly.

The backend communicates with PostgreSQL through the private Docker network:
...
The AWS production environment is provisioned using Terraform.
...
Terraform state and local Terraform files are excluded from Git using .gitignore.

🔄 CI/CD Pipeline

GitHub Actions provides automated validation and production deployment.
...
CI is configured for the main and devops/infrastructure-setup branches.
...
Pushes the image using the immutable Git commit SHA.
Updates the production EC2 environment through AWS Systems Manager.
Retrieves production configuration from AWS Secrets Manager.
Starts the updated Docker containers.
Performs a production health check.

The production image is tagged with the Git commit SHA to provide immutable deployment tracking.

🔐 Security & IAM

Security controls implemented in the DevOps environment include:

GitHub Actions uses OIDC instead of long-lived AWS access keys.
AWS IAM trust is restricted to the project's GitHub repository and deployment branch.
...
CI credentials are stored in GitHub Secrets.
Production deployments use immutable Git commit image tags.
AWS permissions follow least-privilege principles for the implemented deployment workflow.
🚀 Production Environment

The application is deployed to an AWS EC2 instance using Docker.
...
Production endpoints use the EC2 public address:
...
The health endpoint verifies both application availability and database connectivity.

📊 Monitoring & Observability

Amazon CloudWatch is used for production observability.

The CloudWatch Agent collects EC2 system metrics including:
...
ComplianceDocumentReview/EC2

Application Docker logs are collected in:

/compliance-document-review/application

The application log group is configured with a 7-day retention period.

🚨 CloudWatch Alarms

Three production alarms are configured:

AlarmMetricThreshold
ComplianceDocumentReview-HighCPUcpu_usage_user80%
ComplianceDocumentReview-HighMemorymem_used_percent80%
ComplianceDocumentReview-HighDiskdisk_used_percent80%

Each alarm evaluates multiple five-minute periods and treats missing data as not breaching.

Current production alarm state:
...
A production CloudWatch dashboard named:

ComplianceDocumentReview-Production
...
Production implementation evidence was captured during the deployment and monitoring stages.

Recommended evidence files:

docs/screenshots/
├── 01-terraform-infrastructure.png
├── 02-ec2-running.png
├── 03-ssm-connected.png
├── 04-ecr-image.png
...
├── 08-fastapi-docs.png
├── 09-backend-logs.png
├── 10-cloudwatch-metrics.png
├── 11-cloudwatch-alarms.png
└── 12-cloudwatch-dashboard.png

These screenshots provide visual evidence of infrastructure provisioning, deployment, application availability, logging, metrics, alarms, and monitoring.

👨‍💻 DevOps Engineer Contribution

The DevOps implementation covered:
...
The DevOps implementation is considered complete because:
 ...
 Production deployment automated through SSM
 Production health check verified
 CloudWatch logging enabled
 CloudWatch metrics enabled
 CPU, memory, and disk alarms configured
